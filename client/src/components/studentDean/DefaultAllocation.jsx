...
export default function DefaultAllocation({ categorizedStudents }) {
  ...
  useEffect(() => {
    const allocateAllStudents = async () => {
      ...
      console.log("Starting allocation process with blocks:", availableBlocksData);
      ...
      try {
        ...
        if (
          ...
          categorizedStudents.GenderFemale.RegularFemale.NaturalStream.length > 0 ||
    ...
    if (availableBlocksData && categorizedStudents && !isAllocating && !allocationComplete) {
        allocateAllStudents();
    }

  }, [availableBlocksData, categorizedStudents, dispatch, isAllocating, allocationComplete]);
  ...
  const allocateStudentToBlockLogic = (student, block) => {
     ...
     for (let floorIdx = 0; floorIdx < block.floors.length; floorIdx++) {
       const floor = block.floors[floorIdx];
        if (!Array.isArray(floor.dorms)) {
             console.warn(`Block ${block.blockNum}, Floor ${floor.floorNumber} has invalid dorm structure.`);
             continue;
        }
    ...
    const updatedStudent = { /* ... same */
      ...student,
      block: block.blockNum,
      dorm: availableDormInfo.dormNumber,
    };
    const updatedBlock = { /* ... same, includes totalAvailable decrement */
      ...block,
      totalAvailable: (block.totalAvailable || 0) > 0 ? block.totalAvailable - 1 : 0,
              ...
              if (dIdx === availableDormInfo.originalDormIndex) {
                const currentAllocated = Number(dorm.studentsAllocated) || 0;
                 if (currentAllocated >= dorm.capacity) {
                     console.error(`CRITICAL: Attempted to overfill dorm ${dorm.dormNumber} in block ${block.blockNum}. Capacity: ${dorm.capacity}, Already Allocated: ${currentAllocated}`);
                     return dorm;
                 }
  ...
  const allocateCategory = async (students, area, streamType, currentBlockState) => {
    // Base case: No students means success, return state unchanged
    if (!students || students.length === 0) return { success: true, updatedBlocks: currentBlockState };
    ...
    for (const student of sortedStudents) {
      ...
      const candidateBlocksIndices = blocksToProcess
        ...
        .filter(({ block }) => block.location === area && block.totalAvailable > 0)
        .sort((a, b) => a.block.blockNum - b.block.blockNum);

      for (const { block: candidateBlock, index: blockIndex } of candidateBlocksIndices) {
         if(candidateBlock.totalAvailable <= 0) continue;

        const allocationResult = allocateStudentToBlockLogic(student, candidateBlock);
        ...
        try {
          ...
          if (!insertResult.payload?.success) {
             ...
             console.error(`Failed to save allocation for ${student.Fname}: ${insertResult.payload?.message}`);
          ...
          if (!updateResult.payload?.success) {
             console.error(`Failed to update block ${updatedBlock.blockNum} state in DB after allocating ${student.Fname}. Potential inconsistency.`);
          ...
          console.error(`Error during allocation dispatch for ${student.Fname}: ${error.message}`);
      ...
      if (!allocated) {
        console.warn(`Could not allocate student ${student.Fname} (${streamType}, ${area})`);
        allAllocationsSuccessful = false; // Mark batch failure if *any* student fails
      }
    } // End student loop

    console.log(`Finished allocating ${streamType} in ${area}. Success: ${allAllocationsSuccessful}`);
    // Return success status for the batch and the updated block state
    return { success: allAllocationsSuccessful, updatedBlocks: blocksToProcess };
  };

  // Allocates disabled students. Returns { success: boolean, updatedBlocks: Block[] }
  // *** REMOVED TOASTS FROM HERE ***
  const allocateGenderDisabled = async (students, area, genderName, currentBlockState) => {
    ...
    for (const stud of sortedStudents) {
      ...
      const candidateBlocksIndices = blocksToProcess
         ...
         .filter(({ block }) =>
             ...
             block.floors.some(floor =>
                ...
                floor.dorms.some(dorm => (Number(dorm.studentsAllocated)||0) < (Number(dorm.capacity)||0))
             )
         )
         .sort((a, b) => { /* ... same sorting logic ... */
             const aIsSpecial = a.block.isSelectedForSpecial || a.block.isForSpecialNeed;
             const bIsSpecial = b.block.isSelectedForSpecial || b.block.isForSpecialNeed;
      ...
      for (const { block: candidateBlock, index: blockIndex } of candidateBlocksIndices) {
         const firstFloorIndex = candidateBlock.floors.findIndex(f => f.floorNumber === 1);
         ...
         if(Array.isArray(firstFloor.dorms)){
             for(let dIdx = 0; dIdx < firstFloor.dorms.length; dIdx++) {
                 ...
                 if((Number(dorm.studentsAllocated)||0) < (Number(dorm.capacity)||0)) {
         ...
         const currentAllocated = Number(availableDormInfo.studentsAllocated) || 0;
         if (currentAllocated >= availableDormInfo.capacity) { /* ... error check ... */
            console.error(`CRITICAL (Disabled): Attempted to overfill dorm ${availableDormInfo.dormNumber} in block ${candidateBlock.blockNum}.`);
            continue;
         }
         const updatedBlock = { /* ... same */
           ...candidateBlock,
            totalAvailable: (candidateBlock.totalAvailable || 0) > 0 ? candidateBlock.totalAvailable - 1 : 0,
           floors: candidateBlock.floors.map((floor, fIdx) => { /* ... same */
        ...
        try {
          const insertResult = await dispatch( InsertAllocatedStudent({ updatedStudent }) );
          if (!insertResult.payload?.success) {
             console.error(`Failed to save allocation for disabled student ${stud.Fname}: ${insertResult.payload?.message}`);
             allAllocationsSuccessful = false;
             break;
          }
          const updateResult = await dispatch(UpdateBlock({ updatedBlock }));
           if (!updateResult.payload?.success) {
             console.error(`Failed to update block ${updatedBlock.blockNum} state in DB after allocating disabled ${stud.Fname}.`);
             allAllocationsSuccessful = false;
          }
           ...
           console.error(`Error during allocation dispatch for disabled ${stud.Fname}: ${error.message}`);
           allAllocationsSuccessful = false;
           allocated = false;
           break;
        }
      ...
      if (!allocated) {
         console.warn(`Could not allocate disabled student ${stud.Fname} (${genderName}, ${area})`);
        allAllocationsSuccessful = false; // Mark batch failure
      }
    } // End student loop

    console.log(`Finished allocating Disabled ${genderName} in ${area}. Success: ${allAllocationsSuccessful}`);
    // Return success status for the batch and the updated block state
    return { success: allAllocationsSuccessful, updatedBlocks: blocksToProcess };
  };
  ...
  const allocateSpecialStudents = async (currentBlockState) => {
    ...
    if (genderMaleStudents?.length > 0) {
        const { success: maleSuccess, updatedBlocks: stateAfterMale } = await allocateCategory(
            genderMaleStudents, "maleArea", "Special Male", currentState
        );
        if (maleSuccess) {
    ...
    // Female Special
     if (genderFemaleStudents?.length > 0) {
        const { success: femaleSuccess, updatedBlocks: stateAfterFemale } = await allocateCategory(
            genderFemaleStudents, "femaleArea", "Special Female", currentState // Use updated state
        );
         if (femaleSuccess) {
  ...
  const allocateRegularStudents = async (currentBlockState) => {
    ...
    const processStream = async (students, area, streamName, genderName) => {
        ...
        const { success, updatedBlocks } = await allocateCategory(
            students, area, `Regular ${genderName} ${streamName}`, currentState
        );
    ...
    if (maleRegulars) {
        await processStream(maleRegulars.NaturalStream, "maleArea", "Natural", "Male");
        await processStream(maleRegulars.SocialStream, "maleArea", "Social", "Male");
    }
    ...
    const femaleRegulars = categorizedStudents.GenderFemale.RegularFemale;
     if (femaleRegulars) {
        await processStream(femaleRegulars.NaturalStream, "femaleArea", "Natural", "Female");
        await processStream(femaleRegulars.SocialStream, "femaleArea", "Social", "Female");
    }
  ...
  const allocateDisabledStudents = async (currentBlockState) => {
    ...
    if (maleDisabledStudents?.length > 0) {
        const { success: maleSuccess, updatedBlocks: stateAfterMale } = await allocateGenderDisabled(
            maleDisabledStudents, "maleArea", "Male", currentState
        );
        if (maleSuccess) {
    ...
    if (femaleDisabledStudents?.length > 0) {
        const { success: femaleSuccess, updatedBlocks: stateAfterFemale } = await allocateGenderDisabled(
            femaleDisabledStudents, "femaleArea", "Female", currentState // Use updated state
        );
        if (femaleSuccess) {
  ...
  return (
    <div className="bg-sky-200/55 rounded-md p-4 h-max mt-3">
     ...
     {!isAllocating && allocationComplete && (
      ...
      {!isAllocating && allocationComplete && (!availableBlocksData || availableBlocksData.length === 0) && (
